            ...
            {games.length > 0 ? (
              <ul className="space-y-4">
                {games.map((game) => (
                  <li
                    key={game.id}
                    className="flex flex-col sm:flex-row items-start sm:items-center justify-between bg-gray-800 p-4 rounded"
                  >
                    <span className="text-lg mb-2 sm:mb-0">
                      {getDisplayName(game)}
                    </span>
                    <div className="flex flex-wrap gap-2 justify-center sm:justify-end">
                      {roomCodes[game.id] && (
                        <button
                          onClick={() =>
                            navigate(`/game?roomCode=${roomCodes[game.id]}`)
                          }
                          className="px-4 py-2 bg-blue-500 hover:bg-blue-600 text-white rounded whitespace-normal"
                        >
                      ...
                      <button
                        ...
                        className="px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white rounded whitespace-normal"
                      >
                        Játékszoba generálása
                      </button>
                      <button
                        onClick={() => handleEditGame(game)}
                        className="px-4 py-2 bg-green-500 hover:bg-green-600 text-white rounded whitespace-normal"
                      >
                        Szerkesztés
                      </button>
                      <button
                        onClick={() => handleDeleteGame(game)}
                        className="px-4 py-2 bg-red-500 hover:bg-red-600 text-white rounded whitespace-normal"
                      >
                        Törlés
                      </button>
                    </div>
